...
static void stbir__calculate_coefficients_upsample(
    stbir__info *stbir_info, stbir_filter filter, float scale,
    int in_first_pixel, int in_last_pixel, float in_center_of_out,
...
static void stbir__calculate_coefficients_downsample(
    stbir__info *stbir_info, stbir_filter filter, float scale_ratio,
    int out_first_pixel, int out_last_pixel, float out_center_of_in,
    stbir__contributors *contributor, float *coefficient_group) {
  int i;
...
static void stbir__normalize_downsample_coefficients(
    stbir__info *stbir_info, stbir__contributors *contributors,
...
static void stbir__calculate_filters(stbir__info *stbir_info,
  ...
  if (stbir__use_upsampling(scale_ratio)) {
    ...
    for (n = 0; n < total_contributors; n++) {
      ...
      stbir__calculate_coefficients_upsample(
          stbir_info, filter, scale_ratio, in_first_pixel, in_last_pixel,
    ...
    for (n = 0; n < total_contributors; n++) {
      ...
      stbir__calculate_coefficients_downsample(
          stbir_info, filter, scale_ratio, out_first_pixel, out_last_pixel,
          out_center_of_in, stbir__get_contributor(contributors, n),
          stbir__get_coefficient(coefficients, filter, scale_ratio, n, 0));
    }

    stbir__normalize_downsample_coefficients(stbir_info, contributors,
                                             coefficients, filter, scale_ratio,
...
static float *stbir__add_empty_ring_buffer_entry(stbir__info *stbir_info,
  ...
  if (stbir_info->ring_buffer_begin_index < 0) {
    ...
    STBIR__DEBUG_ASSERT(ring_buffer_index !=
...
static void stbir__resample_horizontal_upsample(stbir__info *stbir_info, int n,
                                                float *output_buffer) {
  int x, k;
  ...
  for (x = 0; x < output_w; x++) {
    ...
    STBIR__DEBUG_ASSERT(n0 < stbir_info->input_w +
                                 stbir_info->horizontal_filter_pixel_margin);
    STBIR__DEBUG_ASSERT(n1 < stbir_info->input_w +
                                 stbir_info->horizontal_filter_pixel_margin);
...
static void stbir__decode_and_resample_upsample(stbir__info *stbir_info,
  ...
  if (stbir__use_width_upsampling(stbir_info))
    stbir__resample_horizontal_upsample(
        stbir_info, n, stbir__add_empty_ring_buffer_entry(stbir_info, n));
  else
...
static float *
stbir__get_ring_buffer_scanline(int get_scanline, float *ring_buffer,
                                int begin_index, int first_scanline,
                                int ring_buffer_size, int ring_buffer_length) {
  int ring_buffer_index =
  ...
  case 1:
    for (k = n0; k <= n1; k++) {
      ...
      float *ring_buffer_entry = stbir__get_ring_buffer_scanline(
          ...
          kernel_pixel_width, ring_buffer_length);
      float coefficient =
...
static void stbir__buffer_loop_upsample(stbir__info *stbir_info) {
  ...
  for (y = 0; y < stbir_info->output_h; y++) {
    ...
    stbir__resample_vertical_upsample(stbir_info, y, in_first_scanline,
...
static void stbir__buffer_loop_downsample(stbir__info *stbir_info) {
  ...
  for (y = -pixel_margin; y < max_y; y++) {
    ...
    stbir__resample_vertical_downsample(stbir_info, y, out_first_scanline,
...
#undef STBIR__NEXT_MEMPTR
  ...
  stbir__calculate_filters(
      info, info->horizontal_contributors, info->horizontal_coefficients,
      info->horizontal_filter, info->horizontal_scale, info->horizontal_shift,
      info->input_w, info->output_w);
  stbir__calculate_filters(info, info->vertical_contributors,
  ...
  STBIR__DEBUG_ASSERT(memcmp(overwrite_output_after_pre,
                             &((unsigned char *)output_data)[begin_forbidden],
                             OVERWRITE_ARRAY_SIZE) == 0);
  STBIR__DEBUG_ASSERT(memcmp(overwrite_tempmem_before_pre,
                             &((unsigned char *)tempmem)[-OVERWRITE_ARRAY_SIZE],
                             OVERWRITE_ARRAY_SIZE) == 0);
  STBIR__DEBUG_ASSERT(memcmp(overwrite_tempmem_after_pre,
                             &((unsigned char *)tempmem)[tempmem_size_in_bytes],
                             OVERWRITE_ARRAY_SIZE) == 0);
#endif
